...
export default function ContactPage() {
  ...
  const validate = () => {
    ...
    if (!formData.phoneNumber.trim()) {
      ...
      newErrors.phoneNumber = "Please enter a valid phone number (e.g. +919876543210)";
    }
    ...
    if (!formData.description.trim()) {
      ...
      newErrors.description = "Please describe the film details in at least 10 characters";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (userId && occasion) {
          try {
            const orderResponse = await fetch("https://api.journeysoflife.co/api/orders", {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
                "x-api-key": 'Jol@2026',
            ...
            if (orderResponse.ok) {
              const paymentSessionId = orderData.data?.payment_session_id || orderData.payment_session_id;
              if (paymentSessionId) {
                ...
                console.error("No payment session ID found in order response", orderData);
        ...
        alert(data.message || "Failed to register user. Please check your inputs.");
      }
    } catch (error) {
            ...
            <div className="bg-white rounded-3xl p-8 lg:p-12 shadow-lg border border-[#6E5644]/10 text-center space-y-6">
              ...
              <div className="space-y-3">
                ...
                <p className="mx-auto max-w-md font-sans text-sm leading-6 text-text-gray">
                  Our creative team will review your details. We will contact you at <strong>{formData.email}</strong> or <strong>{formData.phoneNumber}</strong> within 24 hours to plan your custom legacy film.
                </p>
                <div className="mt-4 p-4 bg-bg-cream border border-[#6E5644]/10 rounded-xl text-left text-xs text-text-gray">
                  <p className="font-semibold text-text-dark mb-1">Response details:</p>
                  <p>The registration API returned a success status. Please inspect your browser's Developer Tools Console to view the response payload.</p>
                </div>
              </div>
          ...
          <div className="max-w-[1320px] mx-auto px-6 md:px-10 grid grid-cols-1 lg:grid-cols-2 gap-[60px] items-center relative z-10">
            <div className="max-w-[560px]">
              <Reveal className="flex items-center gap-3.5 mb-4">
                ...
                <span className="text-[11px] tracking-[0.36em] uppercase text-primary font-semibold">Start Your Film</span>
              </Reveal>
              <Reveal delay={0.1}>
            ...
            <Reveal className="relative aspect-[5/4] rounded-[200px_24px_200px_24px] overflow-hidden shadow-[0_40px_90px_-34px_rgba(110,86,68,0.55)]">
              <ImageSlot id="contact-hero" shape="rect" placeholder="Family sharing a warm moment · golden hour" />
        ...
        <section className="py-[90px] pb-[110px] bg-bg-cream">
          <div className="max-w-[1180px] mx-auto px-6 md:px-10 grid grid-cols-1 md:grid-cols-[1.15fr_0.85fr] gap-14 items-start">
            ...
            <Reveal>
              ...
              >
                <h2 className="font-cormorant font-semibold text-[34px] mb-1.5 text-text-dark">Start Your Film</h2>
                <p className="text-[14.5px] text-text-gray mb-7.5">Step 1: Contact Details &amp; Booking Payment</p>

                {/* Name */}
                <div className="mb-4.5">
                  <label className="block">
                    <span className="block text-[11px] tracking-[0.14em] uppercase text-text-gray font-semibold mb-2">Full Name</span>
                    <input
                      required
                      type="text"
                      name="name"
                      ...
                      className={`w-full bg-bg-cream border rounded-xl px-4 py-3.5 text-[14.5px] text-text-dark outline-none transition-colors focus:border-primary ${errors.name ? "border-red-500 bg-red-50/10 focus:border-red-500" : "border-[#6E5644]/18"
                        }`}
                    />
                  </label>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4.5 mb-4.5">
                  <div>
                    <label className="block">
                      <span className="block text-[11px] tracking-[0.14em] uppercase text-text-gray font-semibold mb-2">Phone Number</span>
                      <input
                        required
                        type="tel"
                        name="phoneNumber"
                        ...
                        className={`w-full bg-bg-cream border rounded-xl px-4 py-3.5 text-[14.5px] text-text-dark outline-none transition-colors focus:border-primary ${errors.phoneNumber ? "border-red-500 bg-red-50/10 focus:border-red-500" : "border-[#6E5644]/18"
                          }`}
                      />
                    </label>
                    {errors.phoneNumber && <p className="mt-1 text-xs text-red-500 font-sans">{errors.phoneNumber}</p>}
                  </div>
                  <div>
                    <label className="block">
                      <span className="block text-[11px] tracking-[0.14em] uppercase text-text-gray font-semibold mb-2">Email Address</span>
                      <input
                        required
                        type="email"
                        name="email"
                        ...
                        className={`w-full bg-bg-cream border rounded-xl px-4 py-3.5 text-[14.5px] text-text-dark outline-none transition-colors focus:border-primary ${errors.email ? "border-red-500 bg-red-50/10 focus:border-red-500" : "border-[#6E5644]/18"
                          }`}
                      />
                    </label>
                ...
                <div className="mb-4.5">
                  <label className="block">
                    <span className="block text-[11px] tracking-[0.14em] uppercase text-text-gray font-semibold mb-2">Address</span>
                    <textarea
                      required
                      name="address"
                      rows="2"
                      ...
                      className={`w-full bg-bg-cream border rounded-xl px-4 py-3.5 text-[14.5px] text-text-dark outline-none resize-none transition-colors focus:border-primary ${errors.address ? "border-red-500 bg-red-50/10 focus:border-red-500" : "border-[#6E5644]/18"
                        }`}
                    />
                  </label>
                  {errors.address && <p className="mt-1 text-xs text-red-500 font-sans">{errors.address}</p>}
                </div>
                ...
                <div className="mb-6">
                  <label className="block">
                    ...
                    <textarea
                      ...
                      className={`w-full bg-bg-cream border rounded-xl px-4 py-3.5 text-[14.5px] text-text-dark outline-none resize-y transition-colors focus:border-primary ${errors.description ? "border-red-500 bg-red-50/10 focus:border-red-500" : "border-[#6E5644]/18"
                        }`}
                    />
                  </label>
            ...
            <Reveal delay={0.08} className="flex flex-col gap-7">
              <div className="relative aspect-[4/5] rounded-2xl overflow-hidden shadow-[0_30px_66px_-32px_rgba(110,86,68,0.55)]">
                <ImageSlot id="contact-side" shape="rect" placeholder="Grandparents & grandchildren · warm" />
              ...
              <div className="flex flex-col gap-4.5">
                ...
                <div className="flex items-center gap-3.5">
                  <span className="w-11 h-11 rounded-full bg-bg-tan flex items-center justify-center flex-none">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#B38A42" strokeWidth="1.5">
